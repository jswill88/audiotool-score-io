...
- `packages/midi-to-musicxml`: TypeScript MIDI-to-MusicXML conversion with ranked direct and MuseScore engines.
...
Project docs other than the root `README.md` and `AGENTS.md` now live under `docs/`: use `docs/TODO.md` for the shared checklist, `docs/CODEMAP.md` for navigation, and `docs/deployment/` for deployment runbooks.

`docs/TODO.md` now keeps active backlog items near the top and archives checked-off work in a `Completed` section at the bottom.
...
- Add `docs/CODEMAP.md` as a human-oriented navigation guide and link it from `README.md`.
...
- For future web UI work, treat accessibility as part of done: prefer native semantic controls, ensure every interactive element has an accessible name and state, verify keyboard order/focus, avoid color-only status signals, and update the `docs/TODO.md` Accessibility checklist when new concerns appear.
...
- The MusicXML-to-Audiotool import flow now separates file/title/actions into a `Start From Score` panel and detected score parts into their own top-level `Parts` panel. On wide screens these panels share the first row in two columns, matching the export workflow's project/tracks structure.
- The MusicXML-to-Audiotool warning display groups repeated empty-part warnings into one message, for example `Parts 2, 4, and 6 had no notes and were skipped.`
...
- Empty source tracks in the MusicXML-to-Audiotool import plan use part-facing warning wording instead of exposing the internal MIDI-track wording.
- The browser title, header brand text, and logo accessible title now use the broader app name `Audiotool Score IO`.
- The sign-in panel now shows the two supported conversion directions, `Audiotool → MusicXML` and `MusicXML → Audiotool`, as a minimal intro.
...
- Audiotool export now has two engines on the existing `/audiotool/convert` route. Both paths first export Audiotool tracks to MIDI, then `@midi-to-xml/midi-to-musicxml` runs either `ranked-direct` or `musescore`. The web app defaults to `ranked-direct`; the API defaults to MuseScore when callers omit `engine`. The generic uploaded-MIDI `/convert` endpoint also accepts `?engine=ranked-direct`, so direct notation is reusable beyond Audiotool.
- Ranked direct export still writes a C-major key signature and uses sharp pitch-class spellings. Key selection and context-aware sharp/flat respelling remain follow-up work; changing the selected key should respell the generated MusicXML without transposing pitches.
- Ranked direct MusicXML combines each triplet note's 3:2 `<time-modification>` with explicit `<tuplet type="start">` and `<tuplet type="stop">` notation. Half- through sixty-fourth-note triplet groups therefore display a visible `3`; grouping can include triplet rests.
- Ranked direct rhythm spelling now runs through the executable grammar in `packages/midi-to-musicxml/src/rhythm-grammar.ts`. `direct-musicxml.ts` remains the MIDI/MusicXML rendering shell, while the grammar owns approved templates, `/4`, `/8`, `/16`, compound and odd-meter grouping, readable tie chunks, release-overhang trimming, sub-beat rest absorption, the 2x staccato threshold, trailing 3/8 cleanup, rest-aware 3/8 beams, secondary sixteenth beams, separate beams for each complete triplet set, mixed triplet values, and dotted-whole 12/8 spelling.
- Ranked direct chooses one clef per MIDI track from its median pitch: below MIDI 57 selects bass clef, otherwise treble. Stem direction is clef-aware, using `D3` as the bass-clef middle line and `B4` as the treble-clef middle line. Multi-staff inference and clef changes remain future work.
- `docs/RHYTHM_TEMPLATES.md` remains the human-readable source of truth. The direct package regression suite now exercises the approved 3/4 and 4/4 exceptions, 3/8 beaming/rest rules, triplet-rest simplification, mixed `2t`/`4t` groups, odd-meter fallback groups, and compound long values. Coherent quintuplet/septuplet candidate generation remains a follow-up because the current quantizer only proposes ordinary and triplet grids.
- Next architecture decision: replace the separate MuseScore fixed-grid preprocessing and ranked-direct candidate quantization with one shared multi-grid quantizer. Its canonical quantized MIDI should feed both engines; ranked-direct then applies the rhythm grammar in `docs/RHYTHM_TEMPLATES.md` as a notation-cleanup stage.
...
- `experiments/notation-ranker` remains the offline synthetic report and tuning harness for notation cleanup. It generates the HTML/JSONL comparisons, synthetic clean references, candidate coverage metrics, heuristic/oracle diagnostics, meter-aware 3/4, 6/8, and 9/8 examples, and the 900-example stress set. Production-oriented ranking and MusicXML writing now live in `packages/midi-to-musicxml/src/notation-ranker.ts` and `packages/midi-to-musicxml/src/direct-musicxml.ts`; the experiment remains the place to inspect and tune behavior.
...
npm run notation-ranker:demo -- --examples 30 --seed 20260623 --out tmp/notation-ranker
npm run check
```
...
2. Read `docs/TODO.md` and `AGENTS.md`.
...
```text
Please read AGENTS.md, docs/HANDOFF.md, README.md, docs/TODO.md, and git status. Continue from the current committed state. First verify the app in Docker or Docker dev with a real Audiotool project if possible, then keep docs/TODO.md updated as tasks are completed.
```
