...
- Audiotool export now has two engines on the existing `/audiotool/convert` route. The web app defaults to `ranked-direct`, which applies the reusable notation ranker and writes MusicXML without invoking MuseScore. `musescore` remains selectable as a comparison/fallback and remains the API default for callers that omit `engine`. Direct export currently covers ranked timing cleanup, rests, voices, chords, ties, stems, first-pass beams, and meter-aware duration spelling, including the 6/8 partial-pulse split and 9/8 complete compound-beat preservation rules.
